...
export const userApiSlice = apiSlice.injectEndpoints({
    endpoints: builder => ({
        ...
        getUserConnectionStatus: builder.mutation({
          query({userId}) {
            return {
                url: `/users/statusConnection/${userId}`,
            }
          }
        }),
        updateUserConnectionStatus: builder.mutation({
            query({ id, status }) {
                return {
                    url: `/users/statusActive/${id}?status=${status}`,
                    method: 'PUT'
                }
            }
        })
...
export const {
    ...
    useGetUserConnectionStatusMutation,
    useUpdateUserConnectionStatusMutation,
} = userApiSlice